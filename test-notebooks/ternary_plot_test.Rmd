...
# playerbox <- playerboxscore("~/Dropbox/17-wnba-player/wnba-season-player-feed-06-03-2017.xlsx")
```
...
fantasy_points <- arch_search$arch_input$pbs_summary %>%
  ...
  select(fdfp:games) %>%
  transmute(
    fdfp_per_g = round(fdfp / games, 2),
    dkfp_per_g = round(dkfp / games, 2),
    yfp_per_g = round(yfp / games, 2),
    games = games)
...
for (t in team_summary$Team) {
  team_data <- player_table %>% 
    filter(Team == t) %>% top_n(12, fdfp_per_g)
  ternary_plot(
